  ...
  (fn []
    (try
      ...
      (catch java.io.IOException e
        (println "ERROR BOOTING APPS IN TRAPPERKEEPER: " config)
...
(defn bootstrap-services-with-config
  ...
  (internal/throw-app-error-if-exists!
    (retry (bootstrap-services-retry-fn (fn [config] (tk/boot-services-with-config services config)) config) 3 100)))
